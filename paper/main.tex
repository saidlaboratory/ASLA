...
measurement-error correction to decision accuracy.
...
bootstrap weighted by draw multiplicity. It is conservative here, because
pair-level variance is \zetaRatio\ times the candidate-level component. In
simulation at these components with a known answer, it overstates the standard
error by a factor of \calibBoot\ and the jackknife by \calibJack. The unbiased
U-statistic standard error is within a factor of \calibU. We report the
U-statistic test. The pre-registered bootstrap gives $p=\bootCandP$, and an
earlier, unweighted bootstrap gave $p=\uniqueSetP$. The conclusion does not
...
variance difference of that size is far below the margins it reports.
