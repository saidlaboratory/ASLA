...
reproducing the released decision accuracies exactly before testing. They do
not reproduce. Using the released target column, \gateExact\ of \gateRows\
task--variant rows match, and the largest error is \gateMaxError. No
convention we tried (prediction column, target column or seed mean, tie rule)
recovers them all. The re-test below is therefore exploratory and makes no
claim about the paper's numbers. Against single-scale ranking at the largest
...
scaling-law re-test is exploratory, because the released decision accuracies do
not reproduce from the released predictions.
